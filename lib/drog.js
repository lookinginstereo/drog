...
var Drog = function(options){
    ...
    this.entries.findOne = function(searchOptions){
        var search = searchOptions.urlTitle || "",
            result = null;
        //search entries for matching string... or query.. very nieve
        _u.each(_cachedEntries, function(entry){
            if(search === entry.url.title){
                result = entry;
            }    
        });
        return result;
